...
class TestAsyncBaseWebAPI(IsolatedAsyncioTestCase):
    ...
    def setUp(self) -> None:
        ...
        self.conn_refused_obj = AsyncBaseWebAPI(
            "localhost", "nouser", "nopass", alt_port="9999"
        )
        self.good_secure_obj = AsyncBaseWebAPI(
            "localhost", "nouser", "nopass", secure=True
        )
        self.good_sec_alt_obj = AsyncBaseWebAPI(
            "localhost", "nouser", "nopass", secure=True, alt_port="9999"
        )
        self.good_sec_alt_obj = AsyncBaseWebAPI(
            "localhost", "nouser", "nopass", secure=True, alt_port="9999"
        )
        self.bad_status_obj = AsyncBaseWebAPI("localhost", "nouser", "nopass")
        self.http_status_obj = AsyncBaseWebAPI("httpstat.us", "nouser", "nopass")
        self.basic_auth_obj = AsyncBaseWebAPI(
            "httpbin.org", "fakeuser", "nopass", basic_auth=True, secure=True
        )
    ...
    def test_incorrect_arguments(self) -> None:
        ...
        self.assertRaises(
            ValueError, AsyncBaseWebAPI, "localhost", "nouser", "nopass", secure="Yes"
        )
        self.assertRaises(
            ValueError,
            AsyncBaseWebAPI,
    ...
    async def test_timeout(self) -> None:
        ...
        async with self.http_status_obj as conn:
            ...
            try:
                result = await conn._transaction(
                    "get", "/200", params={"sleep": "5000"}, timeout=timeout
                )
            except BaseException as e:
    ...
    async def test_basic_auth(self) -> None:
        ...
        async with self.basic_auth_obj as conn:
            result = await conn._transaction("get", "/basic-auth/fakeuser/nopass")

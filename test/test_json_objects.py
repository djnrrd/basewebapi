...
child_objects = {
    "name": "Foo",
    "id": 1,
    "friends": [{"name": "Bar", "id": 2}, {"name": "Baz", "id": 3}],
}
bad_text_filter_list = [
    {"name": "Foo", "id": 1},
    {"name": None, "id": 2},
    {"name": "Baz", "id": 3},
]
...
class TestJSONBaseObject(TestCase):
    ...
    def test_from_json(self):
        self.assertRaises(ValueError, JSONBaseObject.from_json, bad_json_object)
        self.assertIsInstance(self.good_object, JSONBaseObject)

    def test__init__(self):
        self.assertRaises(
            KeyError, JSONBaseObject, ["name", "invalid"], **child_objects
        )
...
class TestJSONBaseList(TestCase):
    ...
    def test_from_json(self):
        self.assertRaises(
            ValueError, JSONBaseList.from_json, bad_json_list, JSONBaseObject
        )
        self.assertRaises(
            ValueError, JSONBaseList.from_json, bad_json_list_objects, JSONBaseObject
        )
